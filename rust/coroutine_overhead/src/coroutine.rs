...
#[inline(never)]
pub async fn dot1(m1: &Matrix, m2: &Matrix, m3: &mut Matrix) -> MyResult {
    size_check(m1, m2, m3).await?;
    let NN = m1.n;
...
#[inline(never)]
pub async fn dot2(m1: &Matrix, m2: &Matrix, m3: &mut Matrix) -> MyResult {
    size_check(m1, m2, m3).await?;
    let NN = m1.n;
    l1_fast(NN, m1, m2, m3).await;
    Ok(())
}

#[inline(never)]
pub async fn l1_fast(NN: usize, m1: &Matrix, m2: &Matrix, m3: &mut Matrix) {
    for i in 0..NN {
        l2_fast(NN, i, m1, m2, m3).await;
    }
}

#[inline(never)]
pub async fn l2_fast(NN: usize, i: usize, m1: &Matrix, m2: &Matrix, m3: &mut Matrix) {
    for j in 0..NN {
        l3_fast(NN, i, j, m1, m2, m3).await;
    }
}

#[inline(never)]
pub async fn l3_fast(NN: usize, i: usize, j: usize, m1: &Matrix, m2: &Matrix, m3: &mut Matrix) {
    let mut sum: usize = 0;
    for k in 0..NN {
